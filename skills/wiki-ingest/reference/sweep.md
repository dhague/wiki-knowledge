...
5. **Report summary at end.** Per file: yes / no / skip / never, report returned (for `yes`), or one-line error (for `yes` that failed). Never page-content dump — the `wiki-ingest` procedure's step 7 already returned a manifest and lint findings; this is sweep-level roll-up, keeping a file's findings under that file rather than pooling them.
